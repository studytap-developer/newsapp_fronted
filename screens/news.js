...
//           <Text style={styles.errorTextt}>Check your connection or try again</Text>
//         </View>
//       );
//     }
...
//           >
//             {["JEE", "JNTUK", "JNTUH", "JOBS", "UPSC"].map((category, index) => (
//               <TouchableOpacity
//                 style={styles.button}
//                 key={index}
...
export default class News2 extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      loading: false,
      activeRequests: 0,
      apiCallFailed: [], // Array to track failed API calls
    };
  }
    ...
    fetch(IMAGES_URL)
      ...
      .catch((error) => {
        this.setState((prevState) => ({
          ...
          apiCallFailed: [...prevState.apiCallFailed, "carouselImages"], // Mark carouselImages API as failed
        }));
      })
      .finally(() => {
        this.finishRequest();
  ...
  fetchData = () => {
    ...
    .catch((error) => {
      this.setState((prevState) => ({
        error: error.toString(),
        apiCallFailed: [...prevState.apiCallFailed, "fetchData"], // Mark fetchData API as failed
      }));
    })
    .finally(() => {
      this.finishRequest();
    ...
    return (
      <View style={styles.mainContainer}>
        ...
        <View>
          ...
          <ScrollView style={styles.newsContainer}>
    {newsData &&
      newsData.slice(0, 20).map((item, index) => (
        <View key={index} style={styles.newsItem}>
          <TouchableOpacity
            onPress={() => this.handlePress(item.url || item.link)}
            style={styles.newsContent}
